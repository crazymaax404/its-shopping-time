...
import { useShoppingList, useDeleteListItem, useUpdateListItem } from '@/services/supabase/hooks';
import { useStartPurchase } from '@/services/supabase/hooks';
import { useUIStore } from '@/stores/uiStore';
import { AddItemModal } from './AddItemModal';
import { EditItemModal } from './EditItemModal';
import { ListItem, Button } from '@/components/ui';
import { formatBRL, formatBRLInput, parseBRLInput } from '@/utils/currency';
...
export function HomeScreen() {
  ...
  const { showAddModal, setShowAddModal, showEditModal, setShowEditModal, editingItemId } = useUIStore();
  ...
  const groupedItems = useMemo(() => {
    ...
    const validItems = list.filter((item): item is ShoppingListItem => 
      !!item && typeof item === 'object' && typeof item.id === 'string' && typeof item.category === 'string' && item.category.length > 0
    );
    ...
    const result = CATEGORIES
      .filter((cat) => groups[cat] && Array.isArray(groups[cat]) && groups[cat].length > 0)
      .map((cat) => ({ title: cat, data: groups[cat]!.filter(Boolean) }));
    console.log('[HomeScreen] groupedItems:', JSON.stringify(result.map(s => ({ category: s.title, count: s.data.length }))));
    return result;
  }, [list]);

  const totalItems = list?.length ?? 0;
  const estimatedTotal = list?.reduce((sum, item) => sum + (item.estimated_price || 0), 0) ?? 0;
  ...
  const handleDelete = (id: string) => {
    Alert.alert('Remover item', 'Tem certeza?', [
      { text: 'Cancelar', style: 'cancel' },
      { text: 'Remover', style: 'destructive', onPress: () => deleteItem.mutate(id) },
    ]);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {totalItems > 0 && groupedItems.length > 0 ? (
        <SectionList
          sections={groupedItems.filter(s => s && Array.isArray(s.items) && s.items.length > 0)}
      ...
      <View style={styles.footer}>
        {estimatedTotal > 0 && (
          <Text style={styles.totalLabel}>
            Total estimado: <Text style={styles.totalValue}>{formatBRL(estimatedTotal)}</Text>
          </Text>
        )}
      ...
      <AddItemModal visible={showAddModal} onClose={() => setShowAddModal(false)} onSuccess={refetch} />
      <EditItemModal visible={showEditModal} onClose={() => setShowEditModal(false)} />
    </View>
  );
}
